...
private:
	...
	double m_omega1;
	double m_omega2;
	Eigen::Vector3f m_displacement;//from center of head
public:
...
class MillipedeTail:public Object{
public:
	MillipedeTail(){m_type = NoType;};
	~MillipedeTail(){};
	void InitNeuroNet(MillipedeHead* a_head, RigidCube* a_tail, int a_l_r);
	void UpdateAll(double dt);
	void Draw(int type, const Camera& camera, const Light& light);
	void Output2File(std::ofstream* filestream, int type);
private:
	MillipedeHead* m_head;
	RigidCube* m_tail_section;
	double m_alpha;
	double m_phi;
	double m_target_alpha;
	int m_l_r;//1 left, -1 right
	...
	double m_l1;
	double m_l2;
	Eigen::Vector3f m_displacement;//from center of last section 
public:
	Eigen::Vector3f m_root_position;
	Eigen::Vector3f m_tip_position;
	Drawer* m_Drawer;
